...
description: Build a character end-to-end through the REST API as a player/agent would — the verified request shapes, the per-level tick loop, and the traps that silently eat choices. Use to reproduce a bug, verify an engine change with a real build, or brief a subagent that must drive the API.
