...
whether an owned source can resolve it. For example, an apparently unrestricted druid
substitution domain in LST data may actually be limited by book text and paired with a lost
opposed-domain spell list.
...
| `aeg_evil` | `alderac_entertainment_group/evil/` — `evil_domains.lst`, `evil_feats.lst`. The pack's spells are not from the Evil book; they live in `alderac_entertainment_group/gods/aeg_gods_spells.lst` |
...
- Known LST error: some third-party class files (Mongoose Encyclopaedia Arcane/Divine,
  Bastion Press) use `CL/5+1+((CL+3)/5)` for good saves. Treat that idiom as `good`; a fresh
  PCGen data install may reintroduce the faulty formula even if a local copy was corrected.
