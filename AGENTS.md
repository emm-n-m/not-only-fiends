...
- **Converted characters** → the same test writes every converted `Character` to `PCG_IMPORT_OUTPUT_PATH` (here `{EXTRA_PACKS_PATH}/deceit_characters/NotOnlyFiends/`), overwriting on every run so `git diff` reports sheet-level drift. **Never point this at `CHARACTERS_PATH`, and never have a test write there**: that is the user's live store of real campaign characters, and `CharacterStore` keys files on character name, so a corpus character would silently overwrite a real one.
